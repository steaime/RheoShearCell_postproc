...
def Decimate(arr, q, x=None, bin_spacing='lin', q_meaning='resc_f', xmin=None, xmax=None, return_opts=False):
    ...
    if nbins > 1:
        if x is None:
            ...
            assert len(x)==len(arr), 'x and y arrays must have the same size'
        if nbins >= len(arr):
            if return_opts:
                return x, arr, {}
            else:
            ...
            logging.debug('{0} bin indices generated by digitizing {1} x values over {2} bins (array size: {3})'.format(len(bin_idx), len(x), len(xbins), len(arr)))
